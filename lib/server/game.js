...
// Tandai sesi lobby yang udah lebih dari 24 jam sebagai 'expired'.
// Dipanggil sebelum read/join — lazy expiration, no cron needed.
// Returns: jumlah sesi yang baru kena expire.
export async function expireStaleLobbySessions({ hours = 24 } = {}) {
  try {
    const result = await query(
      `UPDATE game_sessions
       SET status = 'expired', updated_at = NOW()
       WHERE status = 'lobby' AND created_at < (NOW() - INTERVAL ? HOUR)`,
      [hours]
    );
...
export async function getGameSessionByPin(pin) {
  if (!pin) return null;
  // Bersihin sesi expired dulu supaya status field always reflect realita.
  await expireStaleLobbySessions();
