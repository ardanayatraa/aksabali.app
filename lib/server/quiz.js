...
const categoryDefinitions = [
  ...
  {
    id: "swara",
    name: "Pangangge Suara",
    description: "Sandangan vokal: ulu, suku, taleng, pepet, tedung, taleng tedung.",
    groups: ["Swara"]
  },
  {
    id: "angka",
...
export function buildQuizQuestions({ mode = "nyurat", limit = 12, seed = randomUUID(), category = "semua", includeAnswers = false } = {}) {
  ...
  if (mode === "nyurat") {
    ...
    // Quiz Global — tarik soal random dari SEMUA kategori materi.
    // Sengaja abaikan filter category supaya benar-benar mix segala kategori.
    questions = [
      ...anacaraka.map((item, index) =>
        choiceQuestion(item, anacaraka, index % 2 ? "latin-glyph" : "glyph-latin", `${seed}-a-${index}`, mode)
      ),
      ...swara.map((item, index) =>
        choiceQuestion(item, swara, index % 2 ? "latin-glyph" : "glyph-latin", `${seed}-s-${index}`, mode)
      ),
      ...angka.map((item, index) =>
        choiceQuestion(item, angka, index % 2 ? "latin-glyph" : "glyph-latin", `${seed}-n-${index}`, mode)
      ),
      ...gabunganVokal.map((item, index) =>
        choiceQuestion(item, gabunganVokal, index % 2 ? "latin-glyph" : "glyph-latin", `${seed}-g-${index}`, mode)
      ),
      ...kataAksara.map((item, index) =>
        choiceQuestion(item, kataAksara, index % 2 ? "latin-glyph" : "glyph-latin", `${seed}-k-${index}`, mode)
      )
    ];
  } else {
  ...
  // Mode "acak" sengaja skip filter kategori biar selalu global lintas materi.
  const categorized = mode === "acak" ? questions : filterQuestionsByCategory(questions, category);
