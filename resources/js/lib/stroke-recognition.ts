// Port dari Next.js lib/strokeRecognition.js — scoring stroke user vs SVG referensi.
// Pipeline: user draws → sample to TARGET_POINT_COUNT polyline → DTW + Hausdorff vs ref → 5 metric.
// SVG-path sampling pakai DOM (document.createElementNS), jadi BROWSER-ONLY.

import { Geom } from './geometry';
import { pathToPolyline, type Point } from './svg-path-sampler';

export const TARGET_POINT_COUNT = 48;
...
export const StrokeRecognition = {
    pointsToPath(points: StrokePoint[]): string {
    ...
    pathToPoints(pathD: string, count = TARGET_POINT_COUNT): StrokePoint[] {
        if (!pathD) return [];
        if (typeof document !== 'undefined') {
            ...
            return Array.from({ length: count }, (_, index) => {
                const point = (path as SVGPathElement).getPointAtLength((length * index) / Math.max(1, count - 1));
    ...
    pathLength(points: StrokePoint[]): number {
        let total = 0;
    ...
    centroid(points: StrokePoint[]): Point {
        if (!points.length) return { x: 0, y: 0 };
        const total = points.reduce((acc, point) => ({ x: acc.x + point.x, y: acc.y + point.y }), { x: 0, y: 0 });
        return { x: total.x / points.length, y: total.y / points.length };
    },

    smooth(points: StrokePoint[], radius = 2): StrokePoint[] {
    ...
    resample(points: StrokePoint[], count = TARGET_POINT_COUNT): StrokePoint[] {
        ...
        for (let i = 1; i < points.length; i += 1) {
            const current: StrokePoint = { ...points[i] };
    ...
    dtwDistance(a: StrokePoint[], b: StrokePoint[]): number {
    ...
    directedHausdorff(a: StrokePoint[], b: StrokePoint[]): number {
        if (!a.length || !b.length) return Number.POSITIVE_INFINITY;
    ...
    smoothness(points: StrokePoint[]): number {
    ...
    evaluate(userPoints: StrokePoint[], referenceD: string): StrokeEvaluation {
        ...
        const score = clamp(
            shapeScore * 0.42 + positionScore * 0.24 + directionScore * 0.18 + lengthScore * 0.11 + smoothnessScore * 0.05,
        );
