...
import { StrokeRecognition, type StrokeEvaluation, type StrokeMetric, type StrokePoint } from '@/lib/stroke-recognition';

const averageScore = (metrics: StrokeMetric[]): number => {
...
export interface StrokeResult {
    ...
    normalizedStrokes: StrokePoint[][];
    isDemo: boolean;
}

interface Options {
    referencePaths: string[];
    strokeTemplates?: StrokePoint[][][];
    minScore?: number;
    onComplete?: (result: StrokeResult) => void;
    onStrokeComplete?: (idx: number, metric: StrokeMetric) => void;
}

/**
 * Port lengkap dari Next.js hooks/useStrokeRecognizer.js.
 * Handle: stroke detection per pointer event, evaluate vs reference + template,
 * track mistakes + consecutive wrong (auto-hint setelah 3 salah berturut-turut),
 * demo mode (Tampilkan urutan), reset, persist hasil ke onComplete callback.
 */
export function useStrokeRecognizer({
    ...
    const [strokeMetrics, setStrokeMetrics] = useState<StrokeMetric[]>([]);
    const [lastMetric, setLastMetric] = useState<StrokeMetric | null>(null);
    ...
    const metricsRef = useRef<StrokeMetric[]>([]);
    const mistakesRef = useRef(0);
    const rawStrokesRef = useRef<StrokePoint[][]>([]);
    const normalizedStrokesRef = useRef<StrokePoint[][]>([]);

    const buildResult = useCallback(
        (isDemo = false): StrokeResult => {
    ...
    const markWrong = useCallback((userD: string, metric: StrokeMetric) => {
    ...
    const finishAcceptedStroke = useCallback(
        (idx: number, refD: string, metric: StrokeMetric, rawPoints: StrokePoint[], normalizedPoints: StrokePoint[]) => {
            ...
            setUserStrokes((prev) => [
                ...prev,
                { d: refD, status: strokeMissed.current ? 'correctAfterMiss' : 'correct' },
            ]);
            setFeedback(
                strokeMissed.current ? 'correctAfterMiss' : metric.score >= 88 ? 'correct' : 'close',
            );
            setFeedbackMessage(metric.feedbackMessage);
            strokeMissed.current = false;
            consecutiveWrong.current = 0;
    ...
    const evaluateStroke = useCallback(
        (rawPoints: StrokePoint[]) => {
            ...
            const templatePaths = strokeTemplates
                ...
                .filter((points): points is StrokePoint[] => Array.isArray(points) && points.length > 2)
                .map((points) => StrokeRecognition.pointsToPath(points));
            const evaluations: StrokeEvaluation[] = [refD, ...templatePaths].map((candidateD) =>
    ...
    const getSVGPoint = useCallback((event: React.PointerEvent<SVGSVGElement>): StrokePoint => {
    ...
    const isPalmLikeInput = useCallback((event: React.PointerEvent<SVGSVGElement>): boolean => {
    ...
    const triggerHint = useCallback(() => {
        ...
        const hintStroke: UserStroke = { d: referencePaths[currentStrokeIdx], status: 'hint' };
